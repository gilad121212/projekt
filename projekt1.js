...
function card(arr) {
    ...
    garbageImg.src = "https://cdn-icons-png.flaticon.com/512/535/535246.png"
    garbage.appendChild(garbageImg)
    const link = document.createElement("a")
    garbage.appendChild(link)
    link.href = "projekt2.html"
    const pencel = document.createElement("img")
    pencel.classList.add("garbage")
    pencel.id = "pencel"
    pencel.src = "https://simpleicon.com/wp-content/uploads/pencil.png"
    link.appendChild(pencel)
    p(link, arr)
    clickCard(arr, card)
    garbageImg.addEventListener("click", (event) => {
        ...
        const id = arr.id
        console.log(arr)
        const dataBase = nweData.filter((item) => {
            return item.id !== id
        })
...
function men(arr) {
    ...
    men.addEventListener("click", (event) => {
        body.innerHTML = ""
        for (const key of arr) {
            if (key.category === "men's clothing") {
                card(key)
            }
        }
    })
...
function women(arr) {
    ...
    women.addEventListener("click", (event) => {
        body.innerHTML = ""
        for (const key of arr) {
            if (key.category === "women's clothing") {
                card(key)
            }
        }
    })
}
function jewelery(arr) {
    const body = document.getElementById("body")
...
function electronics(arr) {
    const body = document.getElementById("body")
    const electronics = document.getElementById("electronics")
    electronics.addEventListener("click", (event) => {
        body.innerHTML = ""
        for (const key of arr) {
            if (key.category === "electronics") {
                card(key)
            }
        }
    })
...
function p(link, arr1) {
    link.addEventListener("click", (event) => {
        localStorage.setItem("id", JSON.stringify(arr1.id))
    })

}
function clickCard(arr1, card) {
    card.addEventListener("click", (event) => {
        localStorage.setItem("id", JSON.stringify(arr1.id))
        window.location.assign("projekt4.html");
        event.stoppropagation
    })
}
function search(data) {
    const serch = document.getElementById("serchBut")
    const input = document.getElementById("Search")
    serch.addEventListener("click", (event) => {
        body.innerHTML = ""
        for (const arr of data) {
            if (arr.title.includes(input.value)) {
                card(arr)
            }
        }
    })
